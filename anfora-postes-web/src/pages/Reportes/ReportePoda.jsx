// src/pages/Reportes/ReportePoda.jsx
...
const ReportePoda = () => {
  ...
  const handleBuscar = async () => {
    setLoading(true)
    try {
      const datosObtenidos = await reportesService.getReportePoda(filtros)
  ...
  const handleExportar = async () => {
    setExportando(true)
    try {
      await reportesService.exportarReportePoda(filtros)
              ...
              <table className="tabla-reporte">
                ...
                <tbody>
                  {datos.map((registro, index) => (
                    <tr key={registro.id}>
                      <td>{index + 1}</td>
                      <td>{registro.fecha ? new Date(registro.fecha).toLocaleDateString('es-CO') : ''}</td>
