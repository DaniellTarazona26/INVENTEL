...
router.post('/parcial', verificarToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: error.message || 'Error al guardar inventario parcial' })
  }
})
...
router.post('/', verificarToken, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Inventario creado exitosamente', inventario })
  } catch (error) {
    console.error('Error creando inventario:', error)
    res.status(500).json({ success: false, error: error.message || 'Error al crear inventario' })
  }
})

router.patch('/:id/completar', verificarToken, async (req, res) => {
  try {
    const inventario = await inventariosService.completarConOperadores(req.params.id, req.usuario.id)
    res.json({ success: true, message: '✅ Inventario completado exitosamente', inventario })
  } catch (error) {
    console.error('Error completando inventario:', error)
    res.status(500).json({ success: false, error: error.message || 'Error al completar inventario' })
  }
})

router.put('/:id', verificarToken, async (req, res) => {
  try {
    const inventario = await inventariosService.actualizar(req.params.id, req.body, req.usuario.id)
    res.json({ success: true, message: 'Inventario actualizado exitosamente', inventario })
  } catch (error) {
